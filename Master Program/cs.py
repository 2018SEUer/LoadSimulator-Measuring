...
#
def sendStop():
	g_conn_pool[0].sendall('STOP'.encode('utf-8'))
...
def dataProcs(sarData, powerData, sqConn):
	...
	for i in range(l2):
		sum=0
		for j in range(3):
			sum+=float(pdlist[j][i])
		avg=sum/3
		# 本此测试的最终功耗值
		power_res=avg
	# res列表依次存储timestamp、SARdata、POWERdata
	sar_res.extend(power_res)
	res=sar_res
	# 将sar的数据插入数据库中
	# --------------------------sqlite3----------------------------
	curs=sqConn.cursor()
	...
	res[70],res[71],res[72],power_res))

# CS模块工作核心
def cs_main(ser,sqConn):
	for a in (16, 32):								# cores	2levels
		for b in (30, 60, 90):						# freq	3levals
			for c in (0, 50, 100):					# CPU	3levels
				for d in (10, 40, 70, 98):			# MEM	4levels
					for e in (10, 40, 70, 100):		# DISK	4levels
						for f in (10, 40, 70, 100):	# NET	4levels
							paraList=''
							paraList+=str(a)+' '+str(b)+' '+str(c)+' '+str(d)+' '+str(e)+' '+str(f)
							sendLoadLevel(paraList)
							print('当前测试: '+paraList+'\n')
							# wait
							time.sleep(5)
							sarData=[]		# 存储3组数据
